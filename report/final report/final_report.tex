\documentclass[bsc,logo,twoside,singlespacing,notimes]{infthesis}

\usepackage{graphicx}
\usepackage{float}
\usepackage{wrapfig} % Still needed?
\usepackage{listings}
...
\abstract{
\paragraph*{}
Current systems for monitoring IT infrastructure are designed for use
on large networks operated by dedicated IT staff.  They are complex to manage
and often multiple tools are required to obtain a suitable level of monitoring.

\paragraph*{}
This project involves developing a new IT infrastructure monitoring system from
the ground up for use in SME environments.  The aim is to develop a tool that
is powerful enough to monitor a network on its own as well as being simple to
use and manage.

\paragraph*{}
The new system has comprehensive support for time series monitoring as well as
real time alerting which is an area where two separate tools would traditionally
be required.  The entire system can be managed from a responsive web interface
and has extensive support for plugins allowing for the system to be extended to
support monitoring of a variety of different systems.
}
...
%TODO List current tools being investigated
...
\section{Improvements on Existing Tools} %TODO Something
...
%TODO Heighht of rows without vdots
\begin{table}[H]
	\centering
...
\subsection{Plugin Logic} %TODO Better title!
\paragraph*{}
...
\paragraph*{Scheduler Deadlock} %TODO Is this a deadlock?
