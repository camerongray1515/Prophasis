...
the ground up for use in small and medium-sized enterprise (SME) environments.
First it will take a look at a selection of tools that are currently popular
and evaluate them based off of several points that are important in an SME
environment.  From this several key weaknesses with current tools are identified
and potential improvements on these are discussed. The project then discusses
the design and structure of a solution to the identified issues and then
describes the implementation of a new system that is designed for use in SME
environments. It then goes on to explain how the implemented tool was tested
and deployed in real world environments and discusses how it the system behaved
...
\section{Improvements over Existing Tools}
	...
	(SME) environments with limited IT
