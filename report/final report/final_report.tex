...
\chapter{Introduction}
	...
	Many current tools either fall into the category of real time
	...
	graphs and statistics based on it). There is a gap in the market for a
...
\subsection{Time series monitoring and real time alerting}
	...
	out the health of a system. Collectd, as its name suggests is used to collect
...
\subsection{Support for Custom Metrics}
	...
	Nagios, Icinga 2, collectd and Munin all operate in similar ways here through the
	use of simple scripts that can be written in any language.  Both Nagios and Icinga 2
	...
	value are written to stdout and then picked up by the plugin. When passed
	``config'' as a command line argument, Munin plugins must print out information
...
\subsection{Code/Configuration Delivery to Nodes}
	...
	management tools are often not deployed.  Deploying an existing monitoring tool
...
\subsection{How the user configures the system}
	...
	configuration somewhat more accessible however it is complex and still
...
\subsection{How Dependencies are Handled}
	...
	redundancy/fail over is in place, however is a non-trivial process requiring
...
\subsection{Time Series Monitoring \& Real Time Alerting}
	...
	future failures by analysing trends in the collected data.
...
\subsection{Expandability}
	...
	documented structure.  This provides scope for a plugin ``marketplace''
...
\section{What Prophasis is Aimed At}
	...
	between branch offices and a central datacentre.  These environments usually
	contain tens of hosts rather than thousands.  Prophasis is designed to monitor
	networks such as these in a clear and intuitive way.  It is not designed for
	monitoring massive multinational networks with many internationally 
	distributed datacentres containing thousands of machines.

\chapter{Design}
	This chapter will explain the design of Prophasis and discuss various different
...
\paragraph*{Communications Protocol}
	...
	that Prophasis can be operated on most networks problems being caused by
	firewalls.  HTTP also includes methods for transferring plain text as well
	...
	ease versus SSH which would involve having to develop a custom wire protocol.
...
\subsection{Hosts \& Host Groups}
	...
	(Hardware RAID, Contains GPU), location (Datacentre, Edinburgh Office) or any
...
\subsection{Checks}
	...
	CPU load and memory usage across all hosts in the ``Web servers'' host group.
...
\label{methodology-services}
	...
	impact on the availability of a service.

\subsection{Alerts}

	In Prophasis, alerts are set up to communicate with a specific person/group of
	people in a certain situation.  Alerts are defined in terms of to/from state
...
\subsection{Agent}
	...
	agent which may have privileges that the user does not have.
...
\subsubsection{Scheduling}
	...
	then wait for one second before executing the process again.
...
\label{dispatching}
	...
	concerned about other plugins interfering with their plugin while it is being
	executed.  Figure
...
\paragraph*{Communication Between Core and Agent}
	...
	the agent.  The vertically dashed line separates the actions that occur on the
	core on the left from the actions performed by the agent on the right.
...
\subsection{Web Interface}
	...
	mobile devices without any loss of functionality.  Designing it to be responsive
	...
	In order to keep Prophasis as intuitive as possible, the user interface
	contains clear explanations of different functionality as well as help text and
...
\section{Technologies}
	...
	for developing and deploying Prophasis. It allows Prophasis's dependencies to be kept
	totally separate from the system's Python environment, this is particularly
	important for the agent as it ensures that it cannot interfere with
...
\subsection{Flask Web Framework}
	...
	Flask-Login\cite{flask-login} package to provide login and user session
	management functionality.

\subsection{Tornado HTTP Server}

	While Flask provides a built in web server, it is only designed for
	development	use.\cite{flask-deployment}  In order to provide a production
	suitable web server for the agent it was decided to use the Tornado HTTP
	server which uses the uWSGI interface provided by Flask.  Tornado was
...
\subsection{AdminLTE \& Bootstrap}
	...
	interfaces and therefore very little manual work needs to be carried out to
	make the user interface responsive to work well on both desktop and mobile
	devices.
...
\subsection{Handlebars.js}
	...
	are generated after the page has loaded. In Prophasis, Handlebars templates are
	used to render alerts and items that are
	dynamically added to the DOM such as dependencies when editing services or
	intervals when editing schedules.  This means that any HTML markup can be
...
\subsection{Plugin Logic}
	...
	in order to be able to retrieve the appropriate data.  This may involve
...
\section{Database}
	...
	extensive use of SQLAlchemy's \texttt{relationship()}
	function.\cite{sqlalchemy-relationship}  This allows
	...
	object, changing its attributes and committing the session.  It is also
...
\section{Agent}
	...
	The agent is implemented using the Flask web framework to expose an HTTPS API
...
\paragraph*{Plugin Execution}
	...
	\texttt{Plugin} class is then instantiated and its \texttt{get\_data()} method
	is called.  The data returned by the plugin is then returned to the core.
...
\paragraph*{Schedules}
	...
	remove the row its in from the table.  This is all handled purely in frontend
	JavaScript.
...
\subsubsection{Host Health}
	...
	a selection of machines.  This report lists all the hosts in the system
...
\end{figure}
	...
	the host information view as shown in Figure \ref{host-information}.  This
	view clearly shows the reason
...
\section{Error Logging}
	In order to provide notifications about any error conditions or problems that
	occur anywhere in the Prophasis system the web interface provides a system for
	displaying system logs.  The navigation bar contains an item which will display
	the number of unread system logs if any exist as shown in Figure
	\ref{system-logs-unread}.  Clicking this will load the page shown in Figure
	\ref{system-logs-page} which allows the full logs to be seen.
	
\begin{figure}[H]
	\centering
	\caption{Navigation bar showing unread system logs}
	\label{system-logs-unread}
	\includegraphics[scale=1]{assets/screenshots/system-logs-unread.pdf}
\end{figure}

\begin{figure}[H]
	\centering
	\caption{Page showing system logs}
	\label{system-logs-page}
	\includegraphics[scale=0.44]{assets/screenshots/system-logs-page.pdf}
\end{figure}

\chapter{Testing}
	This chapter explains how Prophasis was tested to ensure both correct operation
	and to evaluate its general performance.  It first starts off by explaining
...
\section{Unit Testing}
	...
	a standard Python distribution and is well documented.
	...
	file. The SQLAlchemy ORM transparently handles the differences
...
\section{Mock Agent}
	...
	communicate with it as though were is a regular agent.  It does not use plugins
...
\end{figure}
	...
	load plugin is executed on that specific agent, the new CPU load will be
	sent out.

\section{Use in the Field}

	There is only so much information that can be gained from testing a system such
	as Prophasis in a synthetic environment through the use of the mock agent and unit
...
\subsection{Lynchpin Analytics Limited}
	...
	This is typical of the environment that Prophasis is aimed at and where existing
	...
	installed.  This allowed Prophasis to be tested on a selection of different
...
\end{table}
	...
	had experienced the issue. This data showed the start and end times of the period
...
\paragraph*{Plugin Execution Timeouts}
	...
	policy as described in Subsection \ref{dispatching}).  The solution for this
	was to implement timeouts on the agent so that
...
\paragraph*{Windows Support}
	...
	signals.  This solution works on Windows as well as UNIX-like operating systems.
...
\paragraph*{Ideas for Future Work}
	...
	its MD RAID arrays.  This resulted in extremely high CPU load which sent out
...
\paragraph*{Support for Custom Metrics}
	...
	helps to ensure consistency between and reliability of plugins.

\paragraph*{Classification Threshold Definitions}
	This is an area where Prophasis clearly improves on existing systems.  Many other
	...
	value that represents the collected data. Classification is then performed by user
	provided Lua code on the monitoring server itself.  This means that
	classification logic is all stored in one place and can be edited directly from
...
\paragraph*{Code/Configuration Delivery to Nodes}
	...
	Ansible to manage configuration. Tools such as these may not be in
	...
	the web interface. Prophasis automatically handles the rest.
...
\paragraph*{Distributed Monitoring}
	...
	cores to be operating simultaneously where each monitors its own set of nodes.
...
\paragraph*{External API}
	...
	This would allow external tools to use the data collected by Prophasis.  This
	could allow deeper analysis and better use of the collected data
...
\chapter{Conclusion}
	...
	being usable to predict future issues before they occur.  Existing tools often
	...
	issues arose with hosts being monitored.  If it were to have been deployed for
	longer there would be a
	...
	there is no risk of faults such as deadlocks occurring.


	In conclusion, the project demonstrated that current tools are not best suited
	for use in an SME environment and that it was possible to develop a fit for purpose
