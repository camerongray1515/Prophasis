...
The main part of the project involved developing, deploying and testing from
the ground up a new IT infrastructure monitoring system for use in SME
...
deployed in a real world environment has improved on the problems which were
...
\chapter{Conclusion}
	Current IT infrastructure monitoring solutions are largely aimed at large
	networks operated by dedicated staff.  These tools are generally not suited for
	the small and medium-sized enterprise (SME) sector.  They can be complex to set
	...
	sufficient level of monitoring. Prophasis resolves these issues and provides a
	complete system aimed primarily at the SME sector that is ready for use in a
	...
	expandable for monitoring czustom hardware/software.  Most current tools
	rely on custom scripts with no defined structure to be manually set up on
	remote hosts being monitored.  On the other hand, Prophasis has a clear
	...
	In conclusion, Prophasis improves on the weaknesses found in current tools when
	they are deployed in SME environments.  It has been well tested in both
	synthetic and real world environments and performed reliably in both.  There is
	a large amount of scope for future expansion and enhancements however, in it's
